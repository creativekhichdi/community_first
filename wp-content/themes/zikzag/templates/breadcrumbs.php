...
use Zikzag_Theme_Helper as Zikzag;

if (!class_exists('Zikzag_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
     * @package zikzag\templates
     * @author WebGeniusLab <webgeniuslab@gmail.com>
     * @since 1.0.0
     * @version 1.1.3
     */
    class Zikzag_Breadcrumbs
    {
        ...
        {
			$this->text_home     = esc_html__( 'Home', 'zikzag' ); // text for the 'Home' link
			$this->text_category = esc_html__( 'Archive by Category "%s"', 'zikzag' ); // text for a category page
			$this->text_search   = esc_html__( 'Search Results for "%s" Query', 'zikzag' ); // text for a search results page
			$this->text_tag      = esc_html__( 'Posts Tagged "%s"', 'zikzag' ); // text for a tag page
			$this->text_author   = esc_html__( 'Articles Posted by %s', 'zikzag' ); // text for an author page
			$this->text_404      = esc_html__( 'Error 404', 'zikzag' ); // text for the 404 page
			$this->text_page     = esc_html__( 'Page %s', 'zikzag' ); // text 'Page N'
			$this->text_cpage    = esc_html__( 'Comment Page %s', 'zikzag' ); // text 'Comment Page N'
		}
        public function __construct ()
        {
			...
			if ( is_home() || is_front_page() ) {
				...
				if( class_exists( 'WooCommerce' ) && is_product_category()){
					...
					$out .= zikzag_woocommerce_breadcrumb($args);
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(Zikzag::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep
                            . sprintf($link, $archive_link, esc_html__('Portfolio', 'zikzag'))
                            . $sep
                            . $before
				...
				// Custom Post Type
                    ...
                    && is_product()
                    && function_exists('zikzag_woocommerce_breadcrumb')
                ) {
					$args = [];
					$args['delimiter'] = $sep;
					if ($show_home_link) $out .= $sep;
			...
			echo Zikzag::render_html($out);
		}
    }

	new Zikzag_Breadcrumbs();
}
