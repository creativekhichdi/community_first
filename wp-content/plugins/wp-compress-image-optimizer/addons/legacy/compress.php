...
class wps_local_compress
{
  ...
  public function __construct()
  {
    ...
    if (!empty(self::$settings['on-upload']) && self::$settings['on-upload'] == '1') {
      ...
      #add_filter('wp_generate_attachment_metadata', array($this, 'on_upload_gutenber'), 15, 2);
    }
    ...
    // Overwrite anything above
    #self::$apiURL = 'https://frankfurt.zapwp.net/local/v2/';
    #self::$apiURL = 'https://nyc.zapwp.net/local/v2/';
  ...
  public function on_upload($data, $attachment_id)
  {
    ...
    #$data2 = $this->compress_image($attachment_id, false, self::$settings['retina'], self::$settings['webp'], false, false, 'return');
    ...
    $response = wp_remote_get(self::$siteUrl . '?apikey='.self::$apiParams['apikey'].'&comms_action=compressOnUpload', array('timeout' => 20, 'method' => 'GET', 'sslverify' => false, 'user-agent' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:20.0) Gecko/20100101 Firefox/20.0', 'blocking' => false));

    $this->writeLog('Url call: ' . self::$siteUrl . '?apikey='.self::$apiParams['apikey'].'&comms_action=compressOnUpload');
...
//
//        $heartbeatReady[$imageID] = array('status' => 'restored');
//        update_option('wps_ic_heartbeat_data', $heartbeatReady);
      ...
      $restored = false;
      wp_send_json_error(array('status' => 'failed-to-get-backup', 'apiUrl' => self::$apiURL, 'apikey' => self::$apiParams['apikey'], 'imageID' => $imageID));
    }
  ...
  public function compress_image($imageID, $bulk = true, $retina = true, $webp = true, $just_thumbs = false, $regenerate = true, $output = 'json')
  {
    ...
    $post_fields = array('action' => 'compressArray', 'imageID' => $imageID, 'siteUrl' => site_url(), 'maxWidth' => WPS_IC_MAXWIDTH, 'apikey' => self::$apiParams['apikey'], 'quality' => self::$apiParams['quality'], 'retina' => self::$apiParams['retina'], 'webp' => self::$apiParams['webp'],);
    ...
    # IMPORTANT!!!
    if ($output == 'json') {
      wp_send_json_success(array(self::$apiURL, $post_fields, $response));
    } else {
      return 'done';
    }
//
