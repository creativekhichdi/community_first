jQuery(document).ready(function ($) {
    ...
    $('.button-start-bulk-restore').on('click', function (e) {
        ...
        $.ajax({
            url: ajaxurl, type: 'POST', data: {action: 'wpc_ic_start_bulk_restore'}, success: function (response) {
    ...
    $('.button-start-bulk-compress').on('click', function (e) {
        ...
        $.ajax({
            url: ajaxurl, type: 'POST', data: {action: 'wpc_ic_start_bulk_compress'}, success: function (response) {

                if (response.success == true) {
                    bulkCompressHeartbeat();
                } else {
                    if (response.data == 'bad-apikey') {
                        alert('Seems your API Key is invalid! Please reconnect!');
                    }
                }
            }
        });
            ...
            $.ajax({
                url: ajaxurl, type: 'POST', data: {action: 'wps_ic_bulkRestoreHeartbeat', lastProgress:lastProgress}, success: function (response) {
                    ...
                    if (response.data.status == 'parsing') {
                        ...
                        setTimeout(function(){
                            $.ajax({
                                url: ajaxurl, type: 'POST', data: {action: 'wps_ic_getBulkStats', type: 'restore'}, success: function (response) {
