...
class wps_ic_enqueues extends wps_ic
{
  ...
  public function __construct()
  {
    ...
    if (!empty($_GET['trp-edit-translation']) || !empty($_GET['elementor-preview']) || !empty($_GET['preview']) || !empty($_GET['tatsu']) || (!empty($_GET['fl_builder']) || isset($_GET['fl_builder'])) || !empty($_GET['PageSpeed']) || !empty($_GET['et_fb']) || !empty($_GET['tve']) || !empty($_GET['fb-edit']) || !empty($_GET['bricks']) || !empty($_GET['ct_builder']) || (!empty($_SERVER['SCRIPT_URL']) && $_SERVER['SCRIPT_URL'] == "/wp-admin/customize.php" || strpos($_SERVER['REQUEST_URI'], 'wp-login.php') !== false)) {
      ...
      if (!isset($_GET['wpc_is_amp']) || empty($_GET['wpc_is_amp'])) {
        ...
        if (self::$wst) {
          #add_filter('script_loader_tag', [$this, 'async_parsing_of_js'], 10, 3);
          #add_filter('script_loader_tag', [$this, 'defer_parsing_of_js'], 10, 3);
          #add_filter('script_loader_tag', [$this->js_delay, 'delay_script_replace'], 10, 3);
        }
      }
  ...
  public function is_st()
  {
    ...
    $ip_list = ['102.133.169.66', '104.211.143.8', '104.211.165.53', '104.214.110.135', '104.214.48.247', '104.214.72.101', '104.41.2.19', '13.66.7.11', '13.76.97.224', '13.78.216.56', '13.85.24.83', '13.85.24.90', '13.85.82.26', '13.87.64.140', '13.87.64.168', '13.87.66.254', '13.91.230.174', '157.55.189.189', '172.255.48.130', '172.255.48.131', '172.255.48.132', '172.255.48.133', '172.255.48.134', '172.255.48.135', '172.255.48.136', '172.255.48.137', '172.255.48.138', '172.255.48.139', '172.255.48.140', '172.255.48.141', '172.255.48.142', '172.255.48.143', '172.255.48.144', '172.255.48.145', '172.255.48.146', '172.255.48.147', '191.232.194.51', '191.235.98.164', '191.235.99.221', '20.108.17.87', '20.108.50.109', '20.113.37.60', '20.117.159.236', '20.117.69.107', '20.117.73.245', '20.188.63.151', '20.240.42.3', '20.254.32.214', '20.254.51.234', '20.26.224.131', '20.52.146.77', '20.52.36.49', '20.79.253.26', '20.90.103.62', '20.90.189.200', '20.90.255.148', '20.90.79.252', '20.90.97.209', '23.96.34.105', '24.109.190.162', '40.123.218.94', '40.74.242.253', '40.74.243.13', '40.74.243.176', '40.83.89.214', '51.142.233.172', '51.142.243.236', '51.142.250.140', '51.142.82.146', '51.144.102.233', '52.162.212.163', '52.172.14.87', '52.175.57.81', '52.229.122.240', '52.231.199.170', '52.237.235.185', '52.237.236.145', '52.237.250.73', '52.246.165.153', '65.52.113.236', '65.52.36.250', '70.37.83.240'];
  ...
  public function enqueue_frontend()
  {
    ...
    if ((!empty($options['lazy']) && $options['lazy'] == '1')) {
      ...
      if (self::$settings['css'] == 0 && self::$settings['js'] == 0 && self::$settings['serve']['jpg'] == 0 && self::$settings['serve']['png'] == 0 && self::$settings['serve']['gif'] == 0 && self::$settings['serve']['svg'] == 0) {
        wp_enqueue_script($this::$slug . '-aio', WPS_IC_URI . 'assets/js/dist/optimizer.local-lazy' . WPS_IC_MIN . '.js', ['jquery'], $this::$version);
      } else {
        wp_enqueue_script($this::$slug . '-aio', WPS_IC_URI . 'assets/js/dist/optimizer' . WPS_IC_MIN . '.js', ['jquery'], $this::$version);
      }
      ...
      wp_localize_script($this::$slug . '-aio', 'wpc_vars', ['zoneName' => get_option('ic_cdn_zone_name'), 'siteurl' => site_url(), 'api_url' => 'https://' . self::$zone_name . '/', 'quality' => self::$quality, 'ajaxurl' => admin_url('admin-ajax.php'), 'spinner' => WPS_IC_URI . 'assets/images/spinner.svg', 'background_sizing' => $background_sizing, 'lazy_enabled' => $lazy, 'webp_enabled' => $webp, 'retina_enabled' => $retina, 'force_retina' => $force_retina, 'exif_enabled' => $exif, 'adaptive_enabled' => $adaptive, 'js_debug' => self::$js_debug]);
    } else {
      if (self::$settings['css'] == 0 && self::$settings['js'] == 0 && self::$settings['serve']['jpg'] == 0 && self::$settings['serve']['png'] == 0 && self::$settings['serve']['gif'] == 0 && self::$settings['serve']['svg'] == 0) {
        // Live CDN Disabled
        wp_enqueue_script($this::$slug . '-aio', WPS_IC_URI . 'assets/js/dist/optimizer.local' . WPS_IC_MIN . '.js', ['jquery'], $this::$version);
      } else {
        // Live CDN Enabled
        wp_enqueue_script($this::$slug . '-aio', WPS_IC_URI . 'assets/js/dist/optimizer.adaptive' . WPS_IC_MIN . '.js', ['jquery'], $this::$version);
      }
