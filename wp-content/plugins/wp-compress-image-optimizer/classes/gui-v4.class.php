...
class wpc_gui_v4 extends wps_ic
{
  ...
  public static function checkboxDescription_v4($title = 'Demo', $description = 'Demo', $icon = '', $notify = '', $option = 'default', $locked = false, $value = '1', $configure = false, $tooltip = false, $tooltipPosition = 'left', $beta = false)
  {
    $html = '';
    ...
    //    if ($active) {
    //      $html .= '<input class="form-check-input checkbox mt-0 wpc-ic-settings-v4-checkbox" data-option-name="' .
    //        $optionName . '" type="checkbox" checked="checked" value="1" id="' . $optionName . '" name="' . $optionName . '"  data-recommended="' . $default . '" data-safe="' . $safe . '">';
    //      $html .= '<label for="' . $optionName . '"><span></span></label>';
    //    } else {
    //      $html .= '<input class="form-check-input checkbox mt-0 wpc-ic-settings-v4-checkbox" data-option-name="' .
    //        $optionName . '"  type="checkbox" value="1" id="' . $optionName . '" name="' . $optionName . '"  data-recommended="' . $default . '" data-safe="' . $safe . '">';
    //      $html .= '<label for="' . $optionName . '"><span></span></label>';
    //    }
    ...
    if ($active) {
      $html .= '<label class="wpc-switch">
  <input type="checkbox" class="wpc-ic-settings-v4-checkbox ' . $popup . '" data-popup="' . $popupData . '" checked="checked" value="1" id="' . $optionName_cleaned . '" name="' . $optionName . '"  data-recommended="' . $default . '" data-safe="' . $safe . '" data-connected-slave-option="' . $optionName_cleaned . '"/>
  <span class="wpc-switch-slider wpc-switch-round"></span>
  </label>';
    } else {
      $html .= '<label class="wpc-switch">
  <input type="checkbox" class="wpc-ic-settings-v4-checkbox ' . $popup . '" data-popup="' . $popupData . '" value="1" id="' . $optionName_cleaned . '" name="' . $optionName . '"  data-recommended="' . $default . '" data-safe="' . $safe . '" data-connected-slave-option="' . $optionName_cleaned . '"/>
  <span class="wpc-switch-slider wpc-switch-round"></span>
  </label>';
    }
...
<div class="accordion-item option-item">
  ...
  public static function presetModes()
  {
    ...
    $preset = array('safe' => 'Safe Mode', 'recommended' => 'Recommended', 'custom' => 'Custom');

    if (empty($preset_config)) {
      update_option('wps_ic_preset_setting', 'recommended');
    }

    $html .= '<input type="hidden" name="wpc_preset_mode" value="' . $preset_config . '" />
<div class="wpc-dropdown">
    ...
    if (!empty($zone_name)) {
      $html .= '<div style="flex-direction: column;display: flex;justify-content: center;">
                    ...
                    <div class="setting-value setting-configured cname-configured">
                       Connected Domain: <strong>' . $zone_name . '</strong><br/>
                    </div>
