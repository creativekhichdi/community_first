...
class wpc_upgrader extends wps_ic
{
  ...
  public function upgrade()
  {
    ...
    $default_Settings = ['js' => '0', 'css' => '0', 'css_image_urls' => '0', 'external-url' => '0', 'replace-all-link' => '0', 'emoji-remove' => '0', 'disable-oembeds' => '0', 'disable-gutenber' => '0', 'disable-dashicons' => '0', 'on-upload' => '0', 'defer-js' => '0', 'serve' => ['jpg' => '1', 'png' => '1', 'gif' => '1', 'svg' => '1'], 'search-through' => 'html', 'preserve-exif' => '0', 'background-sizing' => '0', 'remove-render-blocking' => '0', 'minify-css' => '0', 'minify-js' => '0', 'fonts' => '0',];

    foreach ($default_Settings as $name => $defaultValue) {
      if ($name == 'serve') {
        $old_settings['serve'] = [];
        foreach ($defaultValue as $type => $value) {
          if (isset($old_settings['serve_' . $type])) {
            //set to old value
            $old_settings['serve'][$type] = $old_settings['serve_' . $type];
          } else {
            //set to default
            $old_settings['serve'][$type] = $value;
          }
          unset($old_settings['serve_' . $type]);
        }
      } else if (!isset($old_settings[$name]) || empty($old_settings[$name])) {
        $old_settings[$name] = $defaultValue;
      }
    }

    if (!get_option('wps_ic_preset_setting')) {
  ...
  {
    $old_settings = get_option(WPS_IC_SETTINGS);
    $default_Settings = ['js' => '0', 'css' => '0', 'css_image_urls' => '0', 'external-url' => '0', 'replace-all-link' => '0', 'emoji-remove' => '1', 'disable-oembeds' => '1', 'disable-gutenber' => '0', 'disable-dashicons' => '0', 'on-upload' => '0', 'defer-js' => '0', 'serve' => ['jpg' => '1', 'png' => '1', 'gif' => '1', 'svg' => '1'], 'search-through' => 'html'];
