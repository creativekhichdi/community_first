		...
		if ($query->have_posts()) :
		  echo '<section class="wgl_cpt_section">';
			echo '<div class="blog-posts">';
				...
				if (! empty($blog_title) || ! empty($blog_subtitle)) {
				  echo '<div class="wgl_module_title item_title">';
					if (! empty($blog_title)) {
						echo '<h3 class="zikzag_module_title blog_title">', wp_kses( $blog_title, $allowed_html ), '</h3>';
					}
					if (! empty($blog_subtitle)) {
						echo '<p class="blog_subtitle">', wp_kses( $blog_subtitle, $allowed_html ), '</p>';
					}
					if ($blog_layout == 'carousel' && $use_navigation) {
				...
				echo '<div class="container-grid row ', esc_attr($row_class) ,'">',
					\Zikzag_Theme_Helper::render_html($blog_items),
				'</div>';

			echo '</div>';

			if ($blog_navigation == 'pagination') {
				echo \Zikzag_Theme_Helper::pagination('10', $query, $blog_navigation_align);
			}
			...
			if ($blog_navigation == 'load_more') {
				...
				echo \Zikzag_Theme_Helper::load_more($wgl_blog_atts, $name_load_more, $class);
			}
